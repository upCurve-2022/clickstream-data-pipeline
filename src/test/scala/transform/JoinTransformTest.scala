...
class JoinTransformTest extends AnyFlatSpec {
  ...
  "transformDataFrame method()" should "add two extra columns"  in {
    ...
    val inputDf: DataFrame = spark.createDataFrame(spark.sparkContext.parallelize(inputDataSet), StructType(joinedTableSchema))
    ...
    val expectedDf: DataFrame = spark.createDataFrame(spark.sparkContext.parallelize(expectedDataSet), StructType(transformSchema))
